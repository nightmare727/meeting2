...
import com.tiens.meeting.web.code.BaseExceptionCode;
import com.tiens.meeting.web.code.ExceptionCode;
...
public class R<T> {
    ...
    public static <E> R<E> fail(Exception exception) {
        if (exception == null) {
            return fail(DEF_ERROR_MESSAGE);
        }
        return new R<>(ExceptionCode.SYSTEM_BUSY.getCode(), null, exception.getMessage());
    }
